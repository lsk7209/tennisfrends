...
export default function EditBlogPostPage({ params }: EditBlogPostPageProps) {
  ...
  const fetchPost = async () => {
    try {
      ...
      if (response.ok && result.data) {
        ...
        alert(`포스트를 불러오는데 실패했습니다: ${result.error}`);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (response.ok) {
        alert('블로그 포스트가 성공적으로 업데이트되었습니다!');
        router.push('/admin/blog');
      } else {
      ...
      <div className="bg-white border-b border-[#E2E8F0]">
        <div className="max-w-4xl mx-auto px-4 py-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[#0F172A]">블로그 포스트 편집</h1>
              <p className="text-[#334155]">블로그 포스트를 수정하고 업데이트하세요.</p>
            </div>
          </div>
        </div>
      </div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input {...field} placeholder="블로그 포스트 제목을 입력하세요" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
